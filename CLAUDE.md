...
| 채팅 강제 자동스크롤 버그픽스 ✅ | `electron/renderer/scroll-utils.js` + `chat.js` + `index.html` + `style.css` | `scrollToBottom()`이 매 SSE 이벤트마다 무조건 바닥으로 끌어가던 문제 수정 — `isNearBottom()` 게이트로 사용자가 위로 스크롤해 읽는 중이면 자동스크롤 보류 + `#scroll-jump-btn`(↓ 새 메시지) 표시. 본인 발화(`appendUserMessage`)·끼어들기 로컬에코만 `scrollToBottom(true)`로 강제 유지(채팅앱 관례). `supervisor-state.js`와 동일한 UMD 듀얼 export 패턴, 테스트: `tests/renderer/scroll-utils.test.js` + `tests/unit/test_scroll_utils_js.py`. **AA-5 회귀 감사(2026-06-22)**: `scrollToBottom(true)` 강제 호출 3곳(본인 발화·끼어들기 에코·신규 스레드 초기화) 모두 의도된 위치임을 확인 — 자동스크롤 회귀 없음. |
...
| 스레드 사이드바 접이식 그룹 ✅ | `electron/renderer/chat.js` + `index.html` + `style.css` | 탭 → 그룹 접기/펼치기, 스레드 인라인 표시, 그룹별 보관 서브섹션, 빨간 배지. **AA-6 개선(2026-06-22)**: 스레드가 있는 그룹의 "+새 시작" 버튼에 `.tg-new-btn--highlight` 파란 강조 적용(`renderSidebarThreads`에서 동적 toggle) + 기존 스레드 선택 시 입력 플레이스홀더에 "새 업무는 ＋ 새 시작" 힌트 표시 — 신규 스레드 진입점 발견성 개선. |
