...
V-1단계(완료) 위에: 진행도(liveness) 탐지(CPU/stdout/창 응답성) → 에이전트 인루프 판단(스턱 시 구조화 결과 LLM 환류) → 자동 백그라운드 디태치(정당히 긴 작업은 SSE 비블록) → baseline 적응 학습(p50/p90 누적 보정). **자동 백그라운드 디태치는 설계 확정(ADR-0003, 즉시 ack + 백로그 Q/O 인프라 재사용)** — 구현은 별도 카드. 상세: `docs/backlog/pending/V-adaptive-tool-timeout.md`.
