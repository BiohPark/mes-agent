...
> 상태: ✅ 완료 · 기본 내장 타입 보호 + Vault 사용자 정의 타입 오버레이 + 동적 사이드바 렌더링
...
- `agent/tools/task_type.py`: `task_type_create`, `task_type_remove` 2종 도구를 추가했다. 두 도구는 `_risk="mutate"`로 중앙 안전 게이트를 타며, 기본 내장 타입 삭제는 거부한다.
