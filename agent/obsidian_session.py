...
_AUTO_EXEC = (
    ...
    " 클라우드 문서(SharePoint/OneDrive/365 등 URL로만 열리는 문서)는 office_web_open으로 편집화면을 연 뒤"
    " 키보드 단축키(Ctrl+H 찾아바꾸기, Ctrl+S 저장)와 UI Automation으로 편집해라(로컬 파일은 COM 도구가 더 정확)."
    ...
    " [파괴적 작업 안전] run_command/start_process가 'blocked'를 반환하거나 기존 파일을 덮어쓰는 등"
    " 되돌릴 수 없는 작업은, 반드시 ask_user로 확인을 먼저 받은 뒤에만 force=true로 재실행해라."
    " 시스템 보호 경로 쓰기는 차단되니 다른 경로를 사용해라."
